...
const app = new Hono()
  ...
  .post('/chat/ai', async (c) => {
    ...
    // ── Build system prompt — STRICT grounding always ──────────────────────────
    const NOT_FOUND_LINE = 'هذه المعلومة غير موجودة في الكتيبات المفهرسة حالياً. راجع الكتيب الرسمي مباشرة.';
    const strictRules = 'أنت مساعد تقني متخصص في منظومة TLS لمطار OEJN جدة.\n\n' +
    ...
    if (pdfContext.length > 0) {
      ...
      systemPrompt = strictRules +
        '\n=== الكتيبات التقنية المفهرسة ===\n' +
        '(لا يوجد أي محتوى مطابق لهذا السؤال في الكتيبات المفهرسة.)\n\n' +
        'بما أنه لا يوجد محتوى مطابق، التزم حرفياً بالقاعدة رقم 4 وأجب بالجملة المحددة فقط، مهما كان السؤال.';
    }
